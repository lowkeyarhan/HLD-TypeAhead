    ...
    public void flush() {
        ...
        try {
            ...
            for (Map.Entry<String, BufferedEvent> entry : drained.entrySet()) {
                Timestamp ts = Timestamp.from(entry.getValue().lastSeenAt());
                batchArgs.add(
                        new Object[] { entry.getKey(), entry.getValue().count(), entry.getValue().count(), ts, ts });
            }
            ...
            log.info("Flush #{} completed: {} distinct queries, {} total increments. Requests/flushes ratio: {:.2f}",
                    flushes, drained.size(),
                    drained.values().stream().mapToLong(BufferedEvent::count).sum(),
                    ratio);
        } catch (Exception e) {
